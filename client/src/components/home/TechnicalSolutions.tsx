          ...
          {tiers.map((tier, index) => (
            ...
            >
              <button
                ...
                className="w-full bg-background border-2 border-gray-200 rounded-lg p-6 hover:border-[#A4D65E] transition-all duration-300 text-left group"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <h3 className="text-xl font-bold text-foreground mb-1 group-hover:text-[#A4D65E] transition-colors">
                ...
                {expandedTier === tier.id && (
                  ...
                  >
                    <div className="bg-gradient-to-br from-[#f0f7e8] to-white border-2 border-t-0 border-gray-200 rounded-b-lg p-6 space-y-4">
                        ...
                        <ul className="space-y-2">
                          {(language === 'nl' ? tier.specsNl : tier.specsEn).map((spec, idx) => (
                            <li key={idx} className="flex items-start gap-3 text-foreground">
                              <span className="mt-1.5 w-2 h-2 rounded-full bg-[#A4D65E] flex-shrink-0" />
                              <span>{spec}</span>
                            </li>
                          ))}
                        </ul>
                      ...
                      {/* CTA Button */}
                      <div className="pt-4">
                        <a href="/#contact" className="inline-block bg-[#A4D65E] hover:bg-[#8BC34A] text-[#2A3439] font-semibold py-3 px-6 rounded-lg transition-colors duration-300">
