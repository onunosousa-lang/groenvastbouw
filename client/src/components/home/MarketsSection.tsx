          ...
          {markets.map((market, index) => (
            <Card key={index} className="border-none shadow-lg hover:shadow-xl transition-shadow overflow-hidden group">
              <div className="h-48 overflow-hidden relative">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#2a3439]/60 to-transparent flex items-end p-6">
                ...
                <ul className="space-y-3">
                  {market.benefits.map((benefit, i) => (
                    <li key={i} className="flex items-center gap-3 text-foreground">
                      <CheckCircle2 className="text-[#8eb564] flex-shrink-0" size={20} />
                      <span>{benefit}</span>
                    </li>
                  ))}
                </ul>
